...
public:
    ...
    {
        subscription_1 = this->create_subscription<geometry_msgs::msg::Pose>("haptic_to_robot_pose_left",
...
int main(int argc, char **argv)
{
    ...
    rclcpp::Node::SharedPtr node = rclcpp::Node::make_shared("left_pose_tracking_demo");
...
//     std::vector<double> joint_values = {0.443,-0.752,0.006,-2.375,-0.092,1.699,0.761};
    //  std::vector<double> joint_values = {0.939,-0.597,-0.172,-2.255,-0.091,1.772,0.977};
    ...
    target_pose_left.header.frame_id = current_ee_tf.header.frame_id;
    ...
    rclcpp::WallRate loop_rate_2(1);

    while (rclcpp::ok())
    {
        target_pose_left.pose.position.x = v[0];
        target_pose_left.pose.position.y = v[1];
        target_pose_left.pose.position.z = v[2];
        target_pose_left.pose.orientation.w = v[3];
        target_pose_left.pose.orientation.x = v[4];
        target_pose_left.pose.orientation.y = v[5];
        target_pose_left.pose.orientation.z = v[6];

        target_pose_left.header.stamp = node->now();
        target_pose_pub->publish(target_pose_left);
